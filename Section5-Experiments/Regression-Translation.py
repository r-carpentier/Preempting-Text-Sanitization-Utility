...
small_language_models = ["opusMT", "llama3.2-1B", "t5"]
scoring = {
    "r2": "r2",
...
load_folderpath = join(
    os.environ["ROOT_SAVE_FOLDER"], "experiments/multi_news/translation/similarities"
)
...
def perform_regression(
    ...
    # Load SLM Data
    ogtext_slmgentrans_sim = np.array(
        load_pickle(
            load_folderpath, f"OGtextsVS{small_language_model}Gentranslation.pickle"
        )
    )

    ogtext_slmnoisygentrans_sim = load_pickle(
        load_folderpath,
        f"OGtextVS{small_language_model}noisygenTranslationFrom{embedding_model}_embedding_model.pickle",
    )
    if corrected_texts:
        ogtext_slmnoisygentrans_sim = load_pickle(
            load_folderpath,
            f"OGtextVS{small_language_model}noisygenTranslationFrom{embedding_model}_embedding_model_corrected.pickle",
        )

    # Load LLM Data
    ogtext_llmnoisygentrans_sim = load_pickle(
        load_folderpath,
        f"OGtextVS{large_language_model}noisygenTranslationFrom{embedding_model}_embedding_model.pickle",
    )
    if corrected_texts:
        ogtext_llmnoisygentrans_sim = load_pickle(
            load_folderpath,
            f"OGtextVS{large_language_model}noisygenTranslationFrom{embedding_model}_embedding_model_corrected.pickle",
        )
    ...
    assert list(ogtext_slmnoisygentrans_sim.keys()) == list(
        ogtext_llmnoisygentrans_sim.keys()
    ) and list(ogtext_slmnoisygentrans_sim.keys()) == list(ogtext_noisytext_sim.keys())
    nb_texts = ogtext_slmgentrans_sim.shape[0]
    epsilons = list(ogtext_slmnoisygentrans_sim.keys())
    nb_epsilons = len(epsilons)

    # # Selecting particular epsilons (because the full range is not interesting for the regression)
    if embedding_model == "bart":
        epsilons: list[int] = [
            epsilon for epsilon in ogtext_slmnoisygentrans_sim.keys() if epsilon <= 250
        ]
    elif embedding_model == "llama":
        epsilons: list[int] = [
            epsilon for epsilon in ogtext_slmnoisygentrans_sim.keys() if epsilon <= 2000
        ]
    else:
        print("Unknown embedding model, cannot continue.")
    ogtext_slmnoisygentrans_sim = {
        key: ogtext_slmnoisygentrans_sim[key] for key in epsilons
    }
    ogtext_llmnoisygentrans_sim = {
        key: ogtext_llmnoisygentrans_sim[key] for key in epsilons
    }
    ogtext_noisytext_sim = {key: ogtext_noisytext_sim[key] for key in epsilons}
    nb_epsilons = len(epsilons)
    ...
    df2 = pd.DataFrame(
        {
            ...
            "ogtext_slmgentrans_sim": 1 - np.tile(ogtext_slmgentrans_sim, nb_epsilons),
            "ogtext_slmnoisygentrans_sim": 1
            - np.array(list(ogtext_slmnoisygentrans_sim.values())).flatten(),
            # Target
            "ogtext_llmnoisygentrans_sim": 1
            - np.array(list(ogtext_llmnoisygentrans_sim.values())).flatten(),
        }
    )

    target = df2["ogtext_llmnoisygentrans_sim"]
    data = df2.drop(columns=["ogtext_llmnoisygentrans_sim"])
