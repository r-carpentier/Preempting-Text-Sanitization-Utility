...
from utils.tools import sampling_multi_news_texts
...
def my_truncate(row, index):
    if (
        ...
        if "|||||" not in row["document"]:
            print("Warning: Cannot remove header in index ", index)
        else:
...
print("Counting dissimilar indexes ...")
...
print(f"Removing {len(dissimilar_indexes)} dissimilar items")
mnews = mnews.filter(
    lambda example, idx: idx not in dissimilar_indexes, with_indices=True
)

print("Select text with maximum 1024 tokens")
# Get the indexes of texts that are maximum 1024 tokens long (using Bart's tokenize)
...
index_list = []
for index, tokens in enumerate(tqdm(texts_tokens)):
    if len(tokens) <= 1024:
...
# Select random samples to be computed in the experiments
_ = sampling_multi_news_texts(filtered_dataset["document"])
