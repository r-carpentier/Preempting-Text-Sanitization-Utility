...
load_folderpath = join(
    os.environ["ROOT_SAVE_FOLDER"], "experiments/multi_news/summarization/similarities"
)
...
def perform_regression(
    ...
    if embedding_model == "bart":
        epsilons: list[int] = [
            epsilon for epsilon in ogtext_slmnoisygensum_sim.keys() if epsilon <= 100
        ]
    elif embedding_model == "llama":
        epsilons: list[int] = [
            epsilon for epsilon in ogtext_slmnoisygensum_sim.keys() if epsilon <= 1000
        ]
    else:
        print("Unknown embedding model, cannot continue.")
    ogtext_slmnoisygensum_sim = {
        key: ogtext_slmnoisygensum_sim[key] for key in epsilons
    }
    ogtext_llmnoisygensum_sim = {
        key: ogtext_llmnoisygensum_sim[key] for key in epsilons
    }
