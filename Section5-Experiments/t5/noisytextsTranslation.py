...
save_folderpath = join(
    os.environ["ROOT_SAVE_FOLDER"],
    f"experiments/multi_news/translation/t5_translation/On_noisy_texts_with_{embedding_model}_embedding_model",
)
...
# Warmup
_ = translator(prefix + "In the beginning, the universe was created.", max_new_tokens=5)
...
for file in sorted(os.listdir(load_folderpath)):
    ...
    # Prepare texts:
    # - Associate them with the translation prompt
    ...
    messages = [prefix + text for text in texts]
    ...
    for i in range(0, n, batch_size):
        ...
        generated_translations = [output["generated_text"] for output in outputs]
