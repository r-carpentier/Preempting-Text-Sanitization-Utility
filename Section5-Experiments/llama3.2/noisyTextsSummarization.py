...
save_folderpath = join(
    os.environ["ROOT_SAVE_FOLDER"],
    f"experiments/multi_news/summarization/llama3.2-1B_generated_summaries/On_noisy_texts_with_{embedding_model}_embedding_model",
)

if corrected_texts:
    load_folderpath = join(
        os.environ["ROOT_SAVE_FOLDER"],
        f"experiments/multi_news/noisy_texts/{embedding_model}_embedding_model_corrected",
    )
    save_folderpath = join(
        os.environ["ROOT_SAVE_FOLDER"],
        f"experiments/multi_news/summarization/llama3.2-1B_generated_summaries/On_noisy_texts_with_{embedding_model}_embedding_model_corrected",
    )
