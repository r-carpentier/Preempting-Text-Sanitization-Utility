...
def sampling_multi_news_texts(texts: list[str]) -> list[str]:
    filepath = join(
        os.environ["ROOT_SAVE_FOLDER"],
        "experiments/multi_news/texts_indexes_to_compute.npy",
    )
    if not os.path.exists(filepath):
        # Random sampling of 1500 texts to compute
        texts_indexes_to_compute = np.random.choice(
            len(texts), size=1500, replace=False
        )

        parent_dir = os.path.dirname(filepath)
        if not os.path.exists(parent_dir):
            os.makedirs(parent_dir)
        np.save(filepath, texts_indexes_to_compute)

    else:
        texts_indexes_to_compute = np.load(filepath)
